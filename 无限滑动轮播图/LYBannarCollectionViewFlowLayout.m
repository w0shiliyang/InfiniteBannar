...
@implementation LYBannarCollectionViewFlowLayout
{
    NSInteger _index;
}
...
//    CGRectMake(self.collectionView.contentOffset.x, 0, self.collectionView.bounds.size.width, self.collectionView.bounds.size.height);
    //2. 获得这个区域的item
    NSArray *visibleItemArray = [self deepCopyWithArray:[super layoutAttributesForElementsInRect:visibleRect]];
...
- (CGPoint)targetContentOffsetForProposedContentOffset:(CGPoint)proposedContentOffset withScrollingVelocity:(CGPoint)velocity
{
    ...
    // 4. 获取到所有可见区域内的item数组
    NSArray *visibleAttributes = [self deepCopyWithArray:[super layoutAttributesForElementsInRect:visibleRec]];
